...
public class SettingsActivity extends Activity {
	...
	private void importData() {
		AlertDialog.Builder adb = new AlertDialog.Builder(this);
		adb.setTitle("Import Data");
		adb.setMessage(
				"This will import EasyMPG data located at /sdcard/easympg/\n\n" +
				"You should have previously exported your data through EasyMPG.\n" +
				"This is designed to transfer vehicle and fillup data when upgrading from the free version to " +
				"the full version.\n\nFollow these steps to transfer data from free to full verson:\n" +
				"1. Open free version, go to settings, and choose 'Export Data'\n" +
				"2. Click the 'Import' button below\n" +
				"3. Uninstall the free version and enjoy the ad-free, full-featured version!");
		adb.setCancelable(true);
		adb.setPositiveButton("Import", new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int which) {
				if (Environment.getExternalStorageState() != null) {
					try {
						File file = new File(Environment.getExternalStorageDirectory() + "/EasyMPG/export_data.txt");
			            ...
			            reader.close();
			            if (importDataFromFile(getApplicationContext(), data)) {
			            	Toast.makeText(getApplicationContext(), "Import Successful!", Toast.LENGTH_SHORT).show();
			            	onBackPressed();
			            }
			            else
			            	Toast.makeText(getApplicationContext(), "Error Reading Data From File", Toast.LENGTH_SHORT).show();
					} catch (Exception e) {}
				}
			}
		});
	 ...
	 *******************************************************************/
	public static boolean importDataFromFile(Context context, String file) {
		ArrayList<VehicleNew> vehicles = MainActivity.vehicles;
		vehicles.clear();
		ArrayList<FillupNew> fillups;
		ArrayList<Cost> costs;
		SimpleDateFormat sdf = new SimpleDateFormat("EEE MMM d HH:mm:ss z yyyy");
		try {
			Scanner fileScanner = new Scanner(file);
			while (fileScanner.hasNextLine()) {
				if (fileScanner.nextLine().equals("<vehicle>")) {
					String name = fileScanner.nextLine();
					String make = fileScanner.nextLine();
					String model = fileScanner.nextLine();
					int year = Integer.parseInt(fileScanner.nextLine());
					int odometer = Integer.parseInt(fileScanner.nextLine());
					int numOfFillups = Integer.parseInt(fileScanner.nextLine());
					int numOfCosts = 0;
					if (fileScanner.hasNextInt())
						numOfCosts = Integer.parseInt(fileScanner.nextLine());
					VehicleNew v = new VehicleNew(name, make, model, year);
					v.setOdometer(odometer);
					vehicles.add(v);
					fillups = v.getFillups();
					costs = v.getCosts();
						
					for (int i = 0; i < numOfFillups; i++) {
						fileScanner.nextLine(); // skip a line
						double uc = Double.parseDouble(fileScanner.nextLine());
						double tc = Double.parseDouble(fileScanner.nextLine());
						int odom = Integer.parseInt(fileScanner.nextLine());
						String memo = fileScanner.nextLine();
						Calendar c = new GregorianCalendar();
						c.setTime(sdf.parse(fileScanner.nextLine()));
						boolean isComplete = fileScanner.nextLine().equals("true");
						double mpg = Double.parseDouble(fileScanner.nextLine());
						
						FillupNew f = new FillupNew (uc, tc, odom, c, memo, isComplete);
						f.setMPG(mpg);
						fillups.add(f);
					}
					...
					for (int i = 0; i < numOfCosts; i++) {
						...
						cal.setTime(sdf.parse(fileScanner.nextLine()));
						double cost = Double.parseDouble(fileScanner.nextLine());
						int odom = Integer.parseInt(fileScanner.nextLine());
						int reminder = Integer.parseInt(fileScanner.nextLine());
						boolean reminded = fileScanner.nextLine().equals("true");
						boolean notified = fileScanner.nextLine().equals("true");
						Cost c = new Cost(type, description, cost, cal, odom);
						c.setReminder(reminder);
						c.setReminded(reminded);
						c.setNotified(notified);
						costs.add(c);
					}
				}
			}
			MainActivity.saveVehicles(context);
			return true;
		} catch (Exception e) {
			return false;
		}
